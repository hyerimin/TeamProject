...
<title><c:if test="${param.boardType == 'free' }">내가 쓴 글</c:if>
	<c:if test="${param.boardType == 'notice' }">내게 쓴 글</c:if> <c:if
		test="${param.boardType == 'QnA' }">질문 및 답변</c:if></title>
...
<!--       <a href="javascript:location.href='myflist?boardType=free'">내가 쓴 글</a>-->
	...
	<div id="main" style="margin-left: 250px;">
				...
				<div class="collapse navbar-collapse cw" id="navbarResponsive">
					<ul class="navbar-nav ml-auto">
						...
						<li class="nav-item"><a class="nav-link" href="showList">
								나눔보기 </a></li>
						<li class="nav-item"><a class="nav-link" href="free_board"> 게시판 </a></li>
						<li class="nav-item">
		...
		<div class="table_layout">
			<div class="buttons">
                    <button type="button" 
                    onclick="location.href='myBoard?boardType=free&f_writer=${vo.u_name }'" class="btn btn-secondary">내가 쓴 글 보기</button>
                    <button type="button" class="btn btn-secondary"></button>
                    <button type="button" 
                    onclick="location.href='myBoard?boardType=QnA&f_writer=${vo.u_name }'" class="btn btn-secondary">질문 및 답변</button>
			</div>
		</div>
		<!-- //navigation-->
			...
			<table class="table table-sm">
				...
				<tbody>
						...
						<c:forEach items="${list }" var="item" varStatus="idx">
							<tr>
								<c:if test="${item.f_writer  == lvo.u_name }">
		...
		function closeNav() {
		 ...
		 function searching(){
			...
			if(searchValue != null && searchValue != "")
				location.href = "myBoard?boardType="+'${param.boardType}'+"&searchType="+searchType+"&searchValue="+searchValue;
			else
				alert("검색어를 입력해 주세요");
		 }
	</script>
